<section class="flex items-center flex-col justify-center min-h-screen pt-36 pb-8">
    ...
    <form class="w-1/4 mx-auto" method="post"
        action="<?php echo $this->Html->url(array('action' => 'updateProfile')); ?>" enctype="multipart/form-data">
        <?php if (!empty($user['profile_image'])): ?>
        <img class="rounded-full w-32 h-32 mx-auto" src="<?php echo $this->Html->url('/' . $user['profile_image']); ?>"
            alt="Profile Image">
        <?php else: ?>
        <img class="rounded-full w-32 h-32 mx-auto"
            src="https://m.media-amazon.com/images/M/MV5BYWMxNGQyMTItYmUzOC00OTU1LTlhOGMtMGU5MjgwYWRmZjFkXkEyXkFqcGdeQXVyNDUzOTQ5MjY@._V1_.jpg"
            alt="Profile Image">
        <?php endif; ?>

        <div class="mb-5">
            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="profile_image">Upload
                file</label>
            <input
        ...
        <div class="mb-5 mt-5">
            ...
            <input value="<?php echo isset($user['name']) ? h($user['name']) : $currentUser['name']; ?>" type="text"
                id="name" name="name"
        ...
        <div class="relative max-w-sm mb-5">
            ...
            <input id="birthdate" name="birthdate" datepicker datepicker-format="yyyy/mm/dd" datepicker-autohide
                type="text"
                value="<?php echo isset($user['birthdate']) ? h($user['birthdate']) : $currentUser['birthdate']; ?>>"
                class="block bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500  w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        <div class="mb-5">
            ...
            <textarea name="hobby" id="hobby" rows="4"
                ...
                placeholder="Write your hobbies here..."><?php echo isset($user['hobby']) ? h($user['hobby']) : $currentUser['hobby']; ?></textarea>
        </div>
